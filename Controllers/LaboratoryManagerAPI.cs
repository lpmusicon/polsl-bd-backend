    ...
    */
    ...
    public string Get(){
            using (var db = new DatabaseContext()){
                ...
                where le.Status == "Executed" select new ExecutedExaminationList { Result = le.Result, DoctorComment = le.DoctorComment, OrderDate = le.OrderDate, 
    ...
    public string Get(int LaboratoryWorkerId){ // to na wypadek gdyby pracownik laboratorium mogl przegladac tylko swoje wykonane badania, jak moze wszystkie to wyrzucic
            using (var db = new DatabaseContext()){
                var result = (from le in db.LaboratoryExaminations join lw in db.LaboratoryWorkers on le.LaboratoryWorkerId equals lw.LaboratoryWorkerId 
                where le.Status == "Executed" && le.LaboratoryWorkerId == LaboratoryWorkerId select new ExecutedExaminationList { Result = le.Result, DoctorComment = le.DoctorComment, OrderDate = le.OrderDate, 
                ExaminationDate = le.ExaminationDate, Status = le.Status, LabWorkerName = lw.Name, LabWorkerLastname = lw.Lastname}).ToList();  
    ...
    public string Get(){
            using (var db = new DatabaseContext()){
                ...
                where le.Status == "Approval" || le.Status == "Rejected" select new ResolvedExaminationList { Result = le.Result, DoctorComment = le.DoctorComment, OrderDate = le.OrderDate, 
                ExaminationDate = le.ExaminationDate, Status = le.Status, LabWorkerName = lw.Name, LabWorkerLastname = lw.Lastname, LabManagerName = lm.Name, 
                LabManagerLastname = lm.Lastname, ApprovalRejectionDate = le.ApprovalRejectionDate}).ToList();  
